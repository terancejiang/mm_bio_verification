...
class RepVGGDW(torch.nn.Module):
    def __init__(self, ed) -> None:
        ...
        # self.conv = Conv2d_BN(ed, ed, 5, 1, 2, groups=ed)
...
class FaceNet_RepViT(nn.Module):
    def __init__(self, cfgs, num_classes=1000, distillation=False):
        super(FaceNet_RepViT, self).__init__()
        # setting of inverted residual blocks
        self.cfgs = cfgs

        # building first layer
        input_channel = self.cfgs[0][2]
        patch_embed = torch.nn.Sequential(Conv2d_BN(1, input_channel, 2, 2), torch.nn.GELU())
...
def repvit_m0_9(pretrained=False, distillation=False):
    ...
    #
    # ]
    return FaceNet_RepViT(cfgs, distillation=distillation)
...
if __name__ == '__main__':
    ...
    torchinfo.summary(net, input_size=(1, 112, 112), batch_dim=0, verbose=1)
